(function () {
  ...
  function seekToLiveEdge() {
    ...
    } else if (videoEl.buffered && videoEl.buffered.length > 0) {
      videoEl.currentTime = videoEl.buffered.end(videoEl.buffered.length - 1);
    }
    if (videoEl.playbackRate !== 1) videoEl.playbackRate = 1;
  }

  // Auto catch-up: if player falls behind live edge, jump forward
  setInterval(function () {
    if (!hlsInstance || !videoEl || videoEl.paused) return;
    var edge = hlsInstance.liveSyncPosition;
    if (edge && (edge - videoEl.currentTime) > 3) {
  ...
  function setupHls(src) {
    ...
    hlsInstance = new Hls(llHlsConfig);
